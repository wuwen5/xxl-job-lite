...
public class XxlJobRemotingUtil {
    ...
    private static final SSLConnectionSocketFactory SSL_SOCKET_FACTORY =
            new SSLConnectionSocketFactory(SSL_CONTEXT, NoopHostnameVerifier.INSTANCE);
     ...
     */
    public static <T> ReturnT<T> postBody(
            String url, String accessToken, int timeout, Object requestObj, Class<T> returnTargClassOfT) {
        ...
        try (CloseableHttpClient client = getHttpClient(timeout)) {
            ...
            try (CloseableHttpResponse response = client.execute(post)) {
                ...
                if (status != ReturnT.SUCCESS_CODE) {
                    return ReturnT.ofFail("xxl-job remoting fail, status=" + status + ", url=" + resolvedUrl);
                }
            ...
            return ReturnT.ofFail("xxl-job remoting error(" + e.getMessage() + "), url=" + resolvedUrl);
        }
    }
