    ...
    private static void refreshLogReport() {
        for (int i = 0; i < REPORT_DAYS; i++) {
            ...
            Map<String, Object> triggerCountMap =
                    XxlJobAdminConfig.getAdminConfig().getXxlJobLogDao().findLogReport(start, end);

            if (triggerCountMap != null && !triggerCountMap.isEmpty()) {

                Map<String, Object> map = new CaseInsensitiveKeyMap<>();
                map.putAll(triggerCountMap);
                
                int triggerDayCount = map.containsKey("triggerDayCount") ? Integer.parseInt(String.valueOf(map.get("triggerDayCount"))) : 0;
                int triggerDayCountRunning = map.containsKey("triggerDayCountRunning") ? Integer.parseInt(String.valueOf(map.get("triggerDayCountRunning"))) : 0;
                int triggerDayCountSuc = map.containsKey("triggerDayCountSuc") ? Integer.parseInt(String.valueOf(map.get("triggerDayCountSuc"))) : 0;
            ...
            int ret = XxlJobAdminConfig.getAdminConfig().getXxlJobLogReportDao().update(report);
            if (ret < 1) {
                XxlJobAdminConfig.getAdminConfig().getXxlJobLogReportDao().save(report);
            }
        }
    }
