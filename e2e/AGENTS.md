...
**选择器优先级**：`getByRole` > `getByPlaceholder` > `getByText` > `locator(css)`。避免使用易变的 CSS class，优先用语义角色。
...
| 弹窗/对话框 | `page.locator('#someModal')` | `page.getByRole('dialog', { name: '标题' })` |
...
2. **jQuery 式 ID 选择器已不存在**：`#addModal`、`#updateModal`、`#layui-layer1` 等 ID 在 Vue 3 + Element Plus 重构后全部消失。弹窗用 `getByRole('dialog', { name: '标题' })`。
...
- `playwright.config.ts` 中 `timeout: 5000`、`expect.timeout: 3000`，较短——复杂操作需要在测试中显式设置更长超时。
